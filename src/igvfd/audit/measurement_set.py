...
def audit_missing_barcode_replacement_file(value, system):
    ...
    [
        {
            "audit_description": "Measurement sets with a `preferred_assay_title` of Parse SPLiT-seq are expected to have a `barcode_replacement_file`.",
            "audit_category": "missing barcode replacement file",
            "audit_level": "NOT_COMPLIANT"
        },
        {
            "audit_description": "Measurement sets without a `preferred_assay_title` of Parse SPLiT-seq are not expected to have a `barcode_replacement_file`.",
    ...
    if preferred_assay_title == 'Parse SPLiT-seq':
        ...
        if barcode_replacement_file:
            detail = (
                ...
                f'but it is not expected to have one.'
            )
...
def audit_inconsistent_barcode_replacement_file(value, system):
    ...
    [
        {
            "audit_description": "Measurement sets with a `preferred_assay_title` of `Parse SPLiT-seq` are expected to have `barcode_replacement_file` that is a Tabular File with `content_type` of `barcode replacement`.",
