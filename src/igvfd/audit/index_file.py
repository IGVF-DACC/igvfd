...
def audit_index_files_derived_from(value, system):
    ...
    [
        {
            "audit_description": "Index files in tbi format are expected to have a corresponding bed, bedpe, tsv or vcf file in `derived_from`.",
    ...
    if value['file_format'] == 'tbi':
        if derived_from_file_obj.get('file_format') not in ['tsv', 'vcf', 'bed', 'bedpe']:
            detail = (
...
@watch_for_changes_in(functions=list(function_dispatcher_index_file_object.values()))
def audit_index_file_object_dispatcher(value, system):
    yield from run_audits_with_timing(
        function_dispatcher_index_file_object, value, system, frame='object',
    )
