...
def test_summary(testapp, in_vitro_cell_line, in_vitro_differentiated_cell, human_donor, rodent_donor, biomarker_CD243_absent, biomarker_CD243_high, sample_term_lymphoblastoid, sample_term_endothelial_cell, sample_term_embryoid_body, sample_term_brown_adipose_tissue, sample_term_gastrula, treatment_protein, experimental_protocol_document, depletion_treatment, construct_library_set_reporter, crispr_modification):
    ...
    res = testapp.get(in_vitro_differentiated_cell['@id'])
    assert res.json.get(
        'summary') == 'Homo sapiens lymphoblastoid differentiated cell specimen line induced to brown adipose tissue for 5 minutes, transfected with a reporter library targeting accessible genome regions genome-wide (MOI of 2)'
    testapp.patch_json(
        in_vitro_differentiated_cell['@id'],
        {
    ...
    res = testapp.get(in_vitro_differentiated_cell['@id'])
    assert res.json.get(
        'summary') == 'Homo sapiens exhausted lymphoblastoid differentiated cell specimen line induced to brown adipose tissue for 5 minutes, transfected with a reporter library targeting accessible genome regions genome-wide (MOI of 2), grown in DMEM with serum'
    testapp.patch_json(
        in_vitro_differentiated_cell['@id'],
        {
    ...
    testapp.patch_json(
        in_vitro_cell_line['@id'],
        {
            'selection_conditions': ['transfection of target gene']
        }
    )
    res = testapp.get(in_vitro_cell_line['@id'])
    assert res.json.get(
        'summary') == 'virtual Homo sapiens and Mus musculus strain1 (mixed sex) embryoid body pooled differentiated cell specimen (2 donors) induced to gastrula for 5 minutes, (cellular sub pool: PKR-456) (sorting details: some detail about sorting) characterized by high level of CD243, negative detection of CD243, depleted of penicillin for 3 minutes, perturbed with 10 ng/mL G-CSF, modified with CRISPRi Sp-dCas9, transfected with a reporter library targeting accessible genome regions genome-wide, selected by transfection of target gene'
    testapp.patch_json(
        in_vitro_cell_line['@id'],
        {
            'time_post_culture': 5,
...
def test_summary_cell_culture(testapp, in_vitro_cell_culture):
    ...
    res = testapp.get(in_vitro_cell_culture['@id'])
    assert res.json.get('summary') == 'Mus musculus strain1 (male) adrenal gland cell culture cultured for 5 days'
