...
def test_summary(testapp, in_vitro_cell_line, in_vitro_differentiated_cell, human_donor, rodent_donor, biomarker_CD243_absent, biomarker_CD243_high, sample_term_lymphoblastoid, sample_term_endothelial_cell, sample_term_embryoid_body, sample_term_brown_adipose_tissue, treatment_protein, depletion_treatment, construct_library_set_reporter):
    ...
    res = testapp.get(in_vitro_cell_line['@id'])
    assert res.json.get(
        'summary') == f'virtual embryoid body induced to endothelial cell of vascular tree for 3 weeks (cellular sub pool: PKR-456), mixed sex, Homo sapiens and Mus musculus strain1 (sorting details: some detail about sorting) characterized by high level of CD243, negative detection of CD243, depleted of penicillin for 3 minutes, treated with 10 ng/mL G-CSF'
    testapp.patch_json(
        in_vitro_cell_line['@id'],
        {
            'donors': [human_donor['@id'], rodent_donor['@id']],
    ...
    res = testapp.get(in_vitro_cell_line['@id'])
    assert res.json.get(
        'summary') == 'virtual embryoid body induced to brown adipose tissue for 5 minutes (cellular sub pool: PKR-456), mixed sex, Homo sapiens and Mus musculus strain1 (sorting details: some detail about sorting) characterized by high level of CD243, negative detection of CD243, depleted of penicillin for 3 minutes, treated with 10 ng/mL G-CSF, transfected with a reporter library'
    testapp.patch_json(
        in_vitro_differentiated_cell['@id'],
        {
            'moi': 2,
