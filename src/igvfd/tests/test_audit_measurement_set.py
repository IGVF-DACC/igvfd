...
def test_audit_missing_crispr_screen_biometric(
    ...
    measurement_set,
    assay_term_crispr,
    phenotype_term_cell_population_proliferation,
):
    item = {
        'award': award['@id'],
        'lab': lab['@id'],
    ...
    testapp.patch_json(
        crispr_measurement_set['@id'],
        {
            'crispr_screen_biometric': phenotype_term_cell_population_proliferation['@id'],
        },
    )
    res = testapp.get(crispr_measurement_set['@id'] + '@@audit')
    assert all(
...
def test_audit_inconsistent_crispr_screen_biometric(
    ...
    # Proliferation CRISPR screen expects cell population proliferation as crispr_screen_biometric.
    item = {
        'award': award['@id'],
        'lab': lab['@id'],
        'assay_term': assay_term_crispr['@id'],
...
def test_audit_missing_strand_specificity(testapp, measurement_set, assay_term_scrna, sequence_file):
    ...
    # Check if the single-cell RNA-seq measurement set with no strand_specificity and no sequence file(s) is audited (no audit)
    testapp.patch_json(
        measurement_set['@id'],
        {
            'assay_term': assay_term_scrna['@id']
    ...
    # Check if the single-cell RNA-seq measurement set with no strand_specificity and sequence file(s) is audited (yes audit)
    testapp.patch_json(
        sequence_file['@id'],
        {
            'file_set': measurement_set['@id']
