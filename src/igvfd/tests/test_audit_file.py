...
def test_audit_anvil_url(
    testapp,
    principal_analysis_set,
    controlled_access_alignment_file,
    tissue,
    institutional_certificate_controlled
):
    testapp.patch_json(
    ...
    res = testapp.get(controlled_access_alignment_file['@id'] + '@@audit')
    assert all(
        error['category'] != 'missing anvil url'
