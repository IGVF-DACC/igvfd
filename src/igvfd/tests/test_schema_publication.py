...
def test_unique_identifiers(publication, testapp):
    res = testapp.patch_json(
        publication['@id'],
        {'identifier': ['PMID:1000', 'doi:10.1000/100']})
    assert res.status_code == 200
    res = testapp.patch_json(
        publication['@id'],
        {'identifier': ['doi:10.1000/100', 'doi:10.1000/100']}, expect_errors=True)
    assert res.status_code == 422
...
def test_identifier_pattern(publication, testapp):
    res = testapp.patch_json(
        publication['@id'],
        {'identifier': ['PMCID:PMC3439153']})
    assert res.status_code == 200
    res = testapp.patch_json(
        publication['@id'],
        {'identifier': ['PMID:21765801']})
    assert res.status_code == 200
    res = testapp.patch_json(
        publication['@id'],
    ...
    res = testapp.patch_json(
        publication['@id'],
        {'identifier': ['doi:9.1038/nphys1170']}, expect_errors=True)
    assert res.status_code == 422
