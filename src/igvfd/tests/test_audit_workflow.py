...
def test_audit_workflow_without_asvs(testapp, base_workflow_no_asv, analysis_step_version, analysis_step_version_2):
    # Test: workflow without analysis step versions (flag)
    res = testapp.get(base_workflow_no_asv['@id'] + '@@audit')
    assert any(
    ...
    testapp.patch_json(base_workflow_no_asv['@id'], {
