...
def test_differentiated_tissue_upgrade_5_6(upgrader, differentiated_tissue_v4, differentiated_tissue_v4_unknown, differentiated_tissue_v4_90_or_above):
    value = upgrader.upgrade('differentiated_tissue', differentiated_tissue_v4, current_version='4', target_version='5')
    assert value['lower_bound_age'] == 10 and value['upper_bound_age'] == 10
    assert 'life_stage' not in value
    assert value['schema_version'] == '5'
    value = upgrader.upgrade('differentiated_tissue', differentiated_tissue_v4_unknown,
                             current_version='4', target_version='5')
    assert 'life_stage' not in value
    assert 'age' not in value
    assert value['schema_version'] == '5'
    value = upgrader.upgrade('differentiated_tissue', differentiated_tissue_v4_90_or_above,
                             current_version='4', target_version='5')
    assert 'life_stage' not in value
