...
class MeasurementSet(FileSet):
    ...
    @calculated_property(
        ...
        schema={
            ...
            'description': 'The data use limitation summaries of institutional certificates covering the sample associated with this measurement set which are signed by the same lab (or their partner lab) as the lab that submitted this measurement set.',
            'type': 'array',
            'minItems': 1,
            'uniqueItems': True,
            'items': {
    ...
    @calculated_property(
        ...
        schema={
            ...
            'description': 'The controlled access of the institutional certificates covering the sample associated with this measurement set which are signed by the same lab (or their partner lab) as the lab that submitted this measurement set.',
            'type': 'boolean',
            'notSubmittable': True,
        }
    )
    def controlled_access(self, request, lab, samples=None):
        ...
        for sample in samples:
            for ic in sample.get('institutional_certificates', []):
                ...
                ic_labs = ic_object.get('lab', []) + ic_object.get('partner_labs', [])
                if lab in ic_labs:
                    controlled_access_to_return.append(ic_object.get('controlled_access'), None)
