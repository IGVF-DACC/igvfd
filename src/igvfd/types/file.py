...
class File(Item):
    ...
    embedded_with_frame = [
        ...
        Path('workflows', include=['@id', 'uniform_pipeline', 'name']),
    ...
    def workflows(self, request, analysis_step_version):
        """Retrieve the workflows associated with the analysis step version.
        """
        all_workflows = set()
        if analysis_step_version:
            analysis_step_version_object = request.embed(
                analysis_step_version, '@@object_with_select_calculated_properties?field=workflows')
            if analysis_step_version_object.get('workflows'):
                all_workflows.update(analysis_step_version_object['workflows'])
